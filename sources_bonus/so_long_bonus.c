...
/*   Updated: 2021/10/19 15:50:38 by lfornio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (mlx->tab[i])
	{
		...
		while (mlx->tab[i][j])
		{
			...
			if (mlx->tab[i][j] == 'Z')
			{
				...
				if (count <= 6)
					mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->img_ptr_enemy_1, x, y);
				else
